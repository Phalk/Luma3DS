...
void b11WaitCycles(u32 cycles);

void initScreensSequence(u32 brightnessLevel)
{
    CFG11_GPU_CNT2 |= 1;
    CFG11_GPU_CNT |= 0x10000;
    b11WaitCycles(0x86);
    CFG11_GPU_CNT |= 0x1007F;
    CFG11_GPU_CNT3 |= 1;
    *(vu32 *)0x10400004 |= 0x300;
    ...
    *(vu32 *)0x1040059C = 0x00000000;
    ...
    GPIO_DATA3 &= ~(1 << 10);
    GPIO_DATA3_INT_MASK |= (1 << 10);

    GPIO_DATA3 &= ~(1 << 7);
    GPIO_DATA3_INT_MASK |= (1 << 7);
}

void setupFramebuffers(struct fb *fbs)
{
...
void clearScreens(struct fb *fb)
{
...
{
    //Shutdown LCDs (why not ?)
    *(vu32 *)0x10202A44 = 0;
    *(vu32 *)0x10202244 = 0;
    *(vu32 *)0x10202014 = 0;

    //Shutdown GPU
    discardInterrupt(0x58);
    configureAndEnableInterrupt(0x58, 1 << 0, 0);

    if(!(CFG11_GPU_STATUS & (1 << 15)))
    {
        GPU_POWER_CTRL |= 0xF00;
        CFG11_GPU_CNT = ~(1 << 16); //Turn off LCD backlight
        while(!(CFG11_GPU_STATUS & (1 << 15)));
    }

    b11WaitCycles(0x2000);

    //Disable FCRAM for BOTH cores
    CFG11_GPU_FCRAM_CNT &= ~(1 << 1);
    while(CFG11_GPU_FCRAM_CNT & (1 << 2));
    ...
    while(!isInterruptPending(0x58));

    //Enable FCRAM for BOTH cores
    CFG11_GPU_FCRAM_CNT |= 1 << 1;
    while(CFG11_GPU_FCRAM_CNT & (1 << 2));

    acknowledgeInterrupt();
    disableInterrupt(0x58);
}
